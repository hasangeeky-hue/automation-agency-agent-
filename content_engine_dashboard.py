...
def dashboard_html(*, jobs, st, health, month_spent, month_cap, day_spent, day_cap,
    ...
    script = ("<script>window.AGENT_COUNTS=" + _json.dumps(_counts) + ";"
              ...
              "alert('Disconnected — the box is editable again.');keepPlace();}"
              ...
              "if(j.ok){keepPlace();}else{alert(j.error||'refresh failed');if(b){b.disabled=false;b.textContent='↻ Refresh Google data';}}}"
              ...
              "if(j.ok!==false){keepPlace();}else{alert('Failed: '+(j.error||''));if(b){b.disabled=false;b.textContent=old;}}}"
              ...
              "var j=await r.json();alert('Saved. Target CPA per lead: EUR '+((j.targets||{}).target_cpa_lead||'-'));keepPlace();}"
              ...
              "if(j.ok)keepPlace();}catch(e){alert('Failed: '+e);}}"
              ...
              "if(x.ok)keepPlace();}catch(e){alert('Failed: '+e);}}"
              ...
              "keepPlace();}else{alert('Failed: '+(j.error||''));}}"
              "catch(e){alert('Failed: '+e);}}"
              ...
              ".then(function(j){alert(j.message||'Saved');keepPlace();})"
              ...
              "if(j.ok)keepPlace();}).catch(function(e){alert('Failed: '+e);});}"
              ...
              "if(j.ok){keepPlace();}else{alert('Failed: '+(j.error||''));}}catch(e){alert('Failed: '+e);}}"
              ...
              "if(j.ok){alert('✓ Scanned '+(j.competitors||[]).length+' competitors. The board is filled.');keepPlace();}"
              ...
              "alert('✓ Approved'+(note?' with your note':'')+'. It goes live.');keepPlace();}catch(e){alert('Failed: '+e);}}"
              ...
              "alert(j.ok?'↩ Sent back — it will be re-written to fix your note.':'Failed: '+(j.error||''));keepPlace();}catch(e){alert('Failed: '+e);}}"
              ...
              "try{var r=await fetch('/outreach/send_all',{method:'POST'});var j=await r.json();alert('Sent '+j.sent+' of '+j.total+(j.held_by_cap?(' · '+j.held_by_cap+' held by cap (send over coming days)'):''));keepPlace();}catch(e){alert('Failed: '+e);}}"
              "async function trashEmail(job,email){if(!confirm('Delete this email? It moves to the junk box (recoverable, never lost).'))return;"
              "try{await fetch('/outreach/trash',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({job_id:job,email:email})});keepPlace();}catch(e){alert('Failed: '+e);}}"
              "async function restoreEmail(job,email){try{await fetch('/outreach/trash',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({job_id:job,email:email,restore:true})});keepPlace();}catch(e){alert('Failed: '+e);}}"
              ...
              "if(j.ok){alert('Checked replies — '+ (j.added||0) +' new, '+(j.pending||0)+' waiting for you.');keepPlace();}"
              ...
              "alert(j.ok?'✓ Reply sent to the customer.':'Not sent: '+(j.ref||j.error||''));keepPlace();}catch(e){alert('Failed: '+e);}}"
              "async function dismissReply(id){if(!confirm('Dismiss this reply (no answer)?'))return;"
              "try{await fetch('/reply/dismiss',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:id})});keepPlace();}catch(e){alert('Failed: '+e);}}"
              ...
              "if(j.ok){alert('✓ Saved email '+(touch||1)+'. It will preview and send exactly as you wrote it.');keepPlace();}else{alert('Save failed: '+(j.error||''));}}catch(e){alert('Save failed: '+e);}}"
              ...
              "alert(j.ok?('✓ Sent to '+email):('Not sent: '+(j.ref||j.error||'unknown')));keepPlace();}catch(e){alert('Failed: '+e);}}"
              ...
              "alert('Sent '+j.sent+' of '+j.total+(j.held_by_cap?(' · '+j.held_by_cap+' held by the daily warm-up cap'):''));keepPlace();}catch(e){alert('Failed: '+e);}}"
              ...
              "alert('Sent '+j.sent+' of '+j.total+(j.held_by_cap?(' · '+j.held_by_cap+' held by cap'):''));keepPlace();}catch(e){alert('Failed: '+e);}}"
              ...
              "if(j.ok){alert('🚀 Campaign deployed: '+(j.detail||'live in Google Ads'));keepPlace();}else{alert('Could not deploy: '+(j.error||j.detail||'unknown error'));}}"
              ...
              "if(j.ok){alert(j.detail||'Done.');keepPlace();}else{alert('Could not abort: '+(j.error||j.detail||'unknown error'));}}"
              ...
              "if(j.error){alert('Plan failed: '+j.error);if(b){b.disabled=false;b.textContent='🗓️ Plan my week';}}else{alert('Planned '+j.count+' pieces for the week — review the calendar and approve below.');keepPlace();}}"
              ...
              "try{var r=await fetch('/plan/approve',{method:'POST'});var j=await r.json();if(j.error){alert(j.error);}else{alert('✓ Approved — created '+j.created+' pieces. They\\'re now in the pipeline.');keepPlace();}}catch(e){alert('Failed: '+e);}}"
              "async function clearPlan(){if(!confirm('Discard this plan?'))return;try{await fetch('/plan/clear',{method:'POST'});keepPlace();}catch(e){alert('Failed: '+e);}}"
              ...
              "if(j.error){alert('Save failed: '+j.error);}else{alert('Brand saved ('+(j.chars||0)+' chars). Every agent now writes on-brand.');keepPlace();}}catch(e){alert('Save failed: '+e);}}"
              ...
              "if(j.ok){alert('🚀 Autopilot ON. Queued today\\'s work — pieces will publish as they pass QA.');keepPlace();}else{alert('Could not start: '+(j.error||'unknown'));}}catch(e){alert('Failed: '+e);}}"
              "async function stopAutopilot(){try{var r=await fetch('/autopilot/stop',{method:'POST'});var j=await r.json();if(j.ok){alert('■ Autopilot stopped. Nothing new will publish.');keepPlace();}}catch(e){alert('Failed: '+e);}}"
              ...
              "if(j.error){alert('Could not save: '+j.error);}else{keepPlace();}}catch(e){alert('Save failed: '+e);}}"
              ...
              "if(j.error){alert('Eval run failed: '+j.error);}else{alert('Evals: '+j.passed+'/'+j.total+' passed ('+j.score+'%) · cost $'+(j.cost_usd||0).toFixed(3));keepPlace();}}"
              ...
              "if(j.ok){alert('Drafted: '+(j.campaign||'campaign')+'. Scroll down to review it.');keepPlace();}else{alert('Could not draft: '+(j.error||'unknown error'));if(b){b.disabled=false;b.textContent='✍️ Draft a campaign now';}}}"

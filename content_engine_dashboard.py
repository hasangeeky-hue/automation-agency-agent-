...
def dashboard_html(*, jobs, st, health, month_spent, month_cap, day_spent, day_cap,
    ...
    PAGES = [
        ...
        ("osmkt", "📣", "Marketing", "Marketing and Content",
         "The pipeline, the four staff who run it, and the one room where "
         "you approve. Turn 9 of the Agent OS.", _os_mkt_all),
    ...
    # VX2 IS CANCELLED (the founder's word, 2026-08-06). The route still
    # answers at /vx2 for anyone who typed it, but this dashboard no longer
...
def dashboard_script(agent_counts: dict) -> str:
    ...
    return ("<script>window.AGENT_COUNTS=" + _json.dumps(_counts) + ";"
              ...
              # KEEP YOUR PLACE. act() was one of THIRTEEN handlers ending in
              # keepPlace(); the others cover ~615 buttons (runAds,
              # biDeal, sgaCampaign, planContent, runAeo, runSeoAll...). Fixing
              # act() alone would have left the busiest buttons still throwing
              ...
              # A DIALOG IS NOT FEEDBACK. There were 97 alert() calls: each one
              # blocks the page, tells you nothing about WHICH thing changed,
              # and leaves no trace once you dismiss it. This is the same
              # treatment act() got - the message appears, stays readable, and
              # stacks if several things happen. confirm() is untouched: that
              # one is a real decision gate and should block.
              "function toast(msg,ok){try{"
              "var w=document.getElementById('toasts');"
              "if(!w){w=document.createElement('div');w.id='toasts';"
              "w.style.cssText='position:fixed;right:16px;bottom:16px;z-index:9999;"
              "display:flex;flex-direction:column;gap:8px;max-width:380px';"
              "document.body.appendChild(w);}"
              "var t=document.createElement('div');"
              "var good=(ok===undefined)?!/fail|error|could not|refused|invalid/i"
              ".test(String(msg)):!!ok;"
              "t.style.cssText='padding:10px 13px;border-radius:9px;font-size:13px;"
              "line-height:1.45;color:var(--ink);background:var(--s2);"
              "border-left:3px solid '+(good?'var(--good)':'var(--warn)')+';"
              ...
              "var k=sessionStorage.getItem('_kp');if(!k)return;"
              "sessionStorage.removeItem('_kp');var s=JSON.parse(k);"
              "if(s.h&&location.hash!==s.h){location.hash=s.h;}"
              "setTimeout(function(){window.scrollTo(0,s.y||0);},60);"
              "}catch(e){}});"
              # ACT IN PLACE. This used to end in keepPlace(), which
              # rebuilt all 2,227 cards, reset the tab and dropped the scroll
              # position on EVERY action - so pressing a button cost you your
              # place in the work. The button is found from window.event, so
              # all 314 existing call sites keep working unchanged.
              ...
              "var n=card.querySelector('.actout');"
              "if(!n){n=document.createElement('p');n.className='cc actout';"
              "n.style.marginTop='8px';card.appendChild(n);}"
              "n.style.color=ok?'#46E08B':'#FFC24B';"
              ...
              "}catch(e){}}"
              # THE FETCHED RECORD. The calendar carries one compact line per
              # piece; the full record (preview, QA, buttons) is served by
              # /content/record/{id} only when opened. The page stays light
              # at ANY volume - inline embedding is what crashed it.
              "async function seeDetailsFetch(jid){try{"
              "var w=document.getElementById('dlgwrap');"
              "var b=document.getElementById('dlgbody');if(!w||!b)return;"
              "if(_dlgPane)closeDetails();"
              "_dlgFrom=document.activeElement;"
              "b.innerHTML=\"<p class='cc' style='padding:14px'>Loading the "
              "full record\\u2026</p>\";"
              "w.classList.add('on');document.body.style.overflow='hidden';"
              "var r=await fetch('/content/record/'+encodeURIComponent(jid));"
              "b.innerHTML=await r.text();"
              "var p=b.firstElementChild;"
              "var t=document.getElementById('dlgtitle');"
              "if(t&&p)t.textContent=p.getAttribute('data-title')||'Details';"
              "var x=document.getElementById('dlgx');if(x)x.focus();"
              "}catch(e){toast('could not load the record \\u2014 is the "
              ...
              "if(!Object.keys(o).length){toast('Fill in at least one field.');return false;}"
              ...
              "toast('Disconnected — the box is editable again.');keepPlace();}"
              "catch(e){toast('Disconnect failed: '+e);}return false;}"
              ...
              "if(j.ok){keepPlace();}else{toast(j.error||'refresh failed');if(b){b.disabled=false;b.textContent='↻ Refresh Google data';}}}"
              "catch(e){toast('Failed: '+e);if(b){b.disabled=false;b.textContent='↻ Refresh Google data';}}}"
              ...
              "if(j.ok!==false){keepPlace();}else{toast('Failed: '+(j.error||''));if(b){b.disabled=false;b.textContent=old;}}}"
              "catch(e){toast('Failed: '+e);if(b){b.disabled=false;b.textContent=old;}}}"
              ...
              "var j=await r.json();toast('Saved. Target CPA per lead: EUR '+((j.targets||{}).target_cpa_lead||'-'));keepPlace();}"
              ...
              "var j=await r.json();toast(j.ok?j.message:('Failed: '+(j.error||'')));"
              "if(j.ok)keepPlace();}catch(e){toast('Failed: '+e);}}"
              ...
              "var x=await r.json();toast(x.ok?x.message:('Not saved: '+(x.error||'')));"
              "if(x.ok)keepPlace();}catch(e){toast('Failed: '+e);}}"
              ...
              "else{toast('Failed: '+(j.error||''));}}catch(e){toast('Failed: '+e);}}"
              ...
              "var j=await r.json();"
              "if(j.ok){toast('Image attached. Reloading the preview.');"
              "keepPlace();}else{toast('Failed: '+(j.error||''));}}"
              "catch(e){toast('Failed: '+e);}}"
              ...
              "if(!any){toast('Nothing changed.');return;}post('/leads/edit',b);}"
              ...
              ".then(function(j){toast(j.message||'Saved');keepPlace();})"
              ...
              "toast(j.ok?('Saved. '+(j.message||'')):('Could not save: '+(j.error||'unknown')));"
              "if(j.ok)keepPlace();}).catch(function(e){toast('Failed: '+e);});}"
              ...
              "if(j.ok){keepPlace();}else{toast('Failed: '+(j.error||''));}}catch(e){toast('Failed: '+e);}}"
              ...
              "if(j.ok){toast('✓ Scanned '+(j.competitors||[]).length+' competitors. The board is filled.');keepPlace();}"
              "else{toast('Scan failed: '+(j.error||''));if(b){b.disabled=false;b.textContent='🛰️ Scan competitors (~40 credits)';}}}"
              "catch(e){toast('Scan failed: '+e);if(b){b.disabled=false;b.textContent='🛰️ Scan competitors (~40 credits)';}}}"
              ...
              "if(!v.trim()||!c.trim()){toast('Type a business type AND a city first.');return;}"
              ...
              "if(j.ok){toast('✓ '+j.businesses+' businesses sourced · '+j.with_verified_email+' with a verified email.\\n\\n'+(j.next||''));keepPlace();}"
              "else{toast('Failed: '+(j.error||''));if(b){b.disabled=false;b.textContent='🗺️ Find leads';}}}"
              "catch(e){toast('Failed: '+e);if(b){b.disabled=false;b.textContent='🗺️ Find leads';}}}"
              ...
              "toast('✓ Approved'+(note?' with your note':'')+'. It goes live.');keepPlace();}catch(e){toast('Failed: '+e);}}"
              "async function decline(id){var note=_noteFor(id);"
              "if(!note){toast('Please add a note first — tell the system what to fix, then Decline.');return;}"
              ...
              "toast(j.ok?'↩ Sent back — it will be re-written to fix your note.':'Failed: '+(j.error||''));keepPlace();}catch(e){toast('Failed: '+e);}}"
              ...
              "try{var r=await fetch('/outreach/send_all',{method:'POST'});var j=await r.json();toast('Sent '+j.sent+' of '+j.total+(j.held_by_cap?(' · '+j.held_by_cap+' held by cap (send over coming days)'):''));keepPlace();}catch(e){toast('Failed: '+e);}}"
              "async function trashEmail(job,email){if(!confirm('Delete this email? It moves to the junk box (recoverable, never lost).'))return;"
              "try{await fetch('/outreach/trash',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({job_id:job,email:email})});keepPlace();}catch(e){toast('Failed: '+e);}}"
              "async function restoreEmail(job,email){try{await fetch('/outreach/trash',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({job_id:job,email:email,restore:true})});keepPlace();}catch(e){toast('Failed: '+e);}}"
              ...
              "if(j.ok){toast('Checked replies — '+ (j.added||0) +' new, '+(j.pending||0)+' waiting for you.');keepPlace();}"
              "else{toast('Could not read replies: '+(j.error||'')+' (needs IMAP connected).');}}catch(e){toast('Failed: '+e);}}"
              ...
              "toast(j.ok?'✓ Draft saved.':'Save failed: '+(j.error||''));}catch(e){toast('Failed: '+e);}}"
              "async function sendReply(id,i){var s=(document.getElementById('rs-'+i)||{}).value||'';var b=(document.getElementById('rb-'+i)||{}).value||'';"
              "if(b.trim().length<5){toast('The reply looks empty.');return;}"
              ...
              "toast(j.ok?'✓ Reply sent to the customer.':'Not sent: '+(j.ref||j.error||''));keepPlace();}catch(e){toast('Failed: '+e);}}"
              "async function dismissReply(id){if(!confirm('Dismiss this reply (no answer)?'))return;"
              "try{await fetch('/reply/dismiss',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:id})});keepPlace();}catch(e){toast('Failed: '+e);}}"
              ...
              "if(b.trim().length<10){toast('The email body looks too short.');return;}"
              ...
              "if(j.ok){toast('✓ Saved email '+(touch||1)+'. It will preview and send exactly as you wrote it.');keepPlace();}else{toast('Save failed: '+(j.error||''));}}catch(e){toast('Save failed: '+e);}}"
              ...
              "toast(j.ok?('✓ Sent to '+email):('Not sent: '+(j.ref||j.error||'unknown')));keepPlace();}catch(e){toast('Failed: '+e);}}"
              "async function sendSelected(){var sel=Array.prototype.slice.call(document.querySelectorAll('.obx:checked'));if(!sel.length){toast('Tick some emails first.');return;}"
              ...
              "toast('Sent '+j.sent+' of '+j.total+(j.held_by_cap?(' · '+j.held_by_cap+' held by the daily warm-up cap'):''));keepPlace();}catch(e){toast('Failed: '+e);}}"
              "async function sendAllOutbox(){var all=Array.prototype.slice.call(document.querySelectorAll('.obx'));if(!all.length){toast('Nothing ready to send.');return;}"
              ...
              "toast('Sent '+j.sent+' of '+j.total+(j.held_by_cap?(' · '+j.held_by_cap+' held by cap'):''));keepPlace();}catch(e){toast('Failed: '+e);}}"
              "async function runSelftest(){if(!confirm('Test every agent live? ~2 minutes, about $0.25.'))return;toast('Running all 18 agents… I will pop the result when done.');"
              "try{var r=await fetch('/selftest',{method:'POST'});var j=await r.json();var f=(j.failures||[]).map(function(x){return x.skill;}).join(', ');"
              "toast('Agents: '+(j.summary||'done')+(f?('\\nFAILING: '+f):'  — all clean'));}catch(e){toast('Test failed: '+e);}}"
              ...
              "if(j.changed){setTimeout(function(){keepPlace();},1000);}}catch(e){var w2=document.getElementById(wid);if(w2)w2.innerHTML=\"<span class='mut'>Error: \"+e+\"</span>\";}}"
              ...
              "if(j.ok){toast('🚀 Campaign deployed: '+(j.detail||'live in Google Ads'));keepPlace();}else{toast('Could not deploy: '+(j.error||j.detail||'unknown error'));}}"
              "catch(e){toast('Deploy failed: '+e);}}"
              ...
              "if(j.ok){toast(j.detail||'Done.');keepPlace();}else{toast('Could not abort: '+(j.error||j.detail||'unknown error'));}}"
              "catch(e){toast('Abort failed: '+e);}}"
              ...
              "var w=document.getElementById(wid);var t=(j.reply||j.error||'(no reply)').replace(/</g,'&lt;');w.innerHTML=\"<span class='tm' style='min-width:44px'>Agent</span><span class='mut'>\"+t+\"</span>\";log.scrollTop=log.scrollHeight;"
              "if(j.changed){setTimeout(function(){keepPlace();},1200);}}catch(e){var w2=document.getElementById(wid);if(w2)w2.innerHTML=\"<span class='mut'>Error: \"+e+\"</span>\";}}"
              ...
              "if(j.error){toast('Plan failed: '+j.error);if(b){b.disabled=false;b.textContent='🗓️ Plan my week';}}else{toast('Planned '+j.count+' pieces for the week — review the calendar and approve below.');keepPlace();}}"
              "catch(e){toast('Plan failed: '+e);if(b){b.disabled=false;b.textContent='🗓️ Plan my week';}}}"
              ...
              "try{var r=await fetch('/plan/approve',{method:'POST'});var j=await r.json();if(j.error){toast(j.error);}else{toast('✓ Approved — created '+j.created+' pieces. They\\'re now in the pipeline.');keepPlace();}}catch(e){toast('Failed: '+e);}}"
              "async function clearPlan(){if(!confirm('Discard this plan?'))return;try{await fetch('/plan/clear',{method:'POST'});keepPlace();}catch(e){toast('Failed: '+e);}}"
              ...
              "if(j.error){toast('Save failed: '+j.error);}else{toast('Brand saved ('+(j.chars||0)+' chars). Every agent now writes on-brand.');keepPlace();}}catch(e){toast('Save failed: '+e);}}"
              ...
              "if(j.ok){toast('🚀 Autopilot ON. Queued today\\'s work — pieces will publish as they pass QA.');keepPlace();}else{toast('Could not start: '+(j.error||'unknown'));}}catch(e){toast('Failed: '+e);}}"
              "async function stopAutopilot(){try{var r=await fetch('/autopilot/stop',{method:'POST'});var j=await r.json();if(j.ok){toast('■ Autopilot stopped. Nothing new will publish.');keepPlace();}}catch(e){toast('Failed: '+e);}}"
              "async function setApiLimit(api){var el=document.getElementById('lim-'+api);if(!el)return;var v=parseFloat(el.value);if(!(v>0)){toast('Enter a number.');return;}"
              ...
              "if(j.error){toast('Could not save: '+j.error);}else{keepPlace();}}catch(e){toast('Save failed: '+e);}}"
              ...
              "if(j.error){toast('Eval run failed: '+j.error);}else{toast('Evals: '+j.passed+'/'+j.total+' passed ('+j.score+'%) · cost $'+(j.cost_usd||0).toFixed(3));keepPlace();}}"
              "catch(e){toast('Eval run failed: '+e);}if(b){b.disabled=false;b.textContent='▶ Run evals';}}"
              ...
              "if(j.ok){toast('Drafted: '+(j.campaign||'campaign')+'. Scroll down to review it.');keepPlace();}else{toast('Could not draft: '+(j.error||'unknown error'));if(b){b.disabled=false;b.textContent='✍️ Draft a campaign now';}}}"
              "catch(e){toast('Draft failed: '+e);if(b){b.disabled=false;b.textContent='✍️ Draft a campaign now';}}}"
              "async function approveAll(ids){var a=ids.split(',');for(var i=0;i<a.length;i++){try{await fetch('/jobs/'+a[i]+'/approve',{method:'POST'});}catch(e){}}keepPlace();}"
...
if __name__ == "__main__":
    ...
    # Definitions are collected from the WHOLE page, not just the main
    # script block: the SEO audit screens ship their handlers (s2open,
    # s3run, seoAutoSet ...) in their own script tag inside the section,
    ...
    # The SEO audit screens carry their stylesheet WITH the section (the
    # bridge + content_engine_seo_screens.CSS travel in a <style> block
    # inside seo_section's output). Rules embedded in the page style an
    # element exactly as well as rules in the main sheet, so coverage is
    # judged against every <style> block the page actually ships.
    for _blk in _re2.findall(r"<style>(.*?)</style>", html, _re2.S):
        _allcss += _blk
